...
export const loginWithAppleService = async (
  ...
  let profile = await getProfileService(data.user.id);

  if (!profile) {
    profile = await createProfileService(data.user.id, {
      full_name: data.user.user_metadata?.full_name || "Apple User",
    });
  }

  return mapToLoginDTO(
    { ...data, user: { ...data.user, provider: "apple" } },
    profile
  ) as Login;
};
