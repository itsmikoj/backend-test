...
import { AppleLoginInput } from "../validators/auth/authApple";

dotenv.config();

export const registerUserService = async (
  body: PostRegisterInput
): Promise<Register> => {
...
export const updateUserService = async (
  ...
  if (body.username === undefined && body.full_name === undefined) {
    throw new Error("No fields provided to update");
  }

  const profile = await updateProfileService(id, body);
...
export const deleteUserService = async (id: string): Promise<null> => {
  ...
  if (authError) {
    console.log("Error in delete user->", authError);

    throw new Error(`DB: ${authError?.message || "Unknown error"}`);
  }
...
export const loginWithAppleService = async (
  body: AppleLoginInput
): Promise<Login> => {
  const { identityToken, full_name } = body;
  ...
  if (error) throw new Error(`Error in signInWithIdToken: ${error.message}`);
  if (!data || !data.user) throw new Error("Apple login failed");

  const profile = await createProfileService(data.user.id, {
    full_name: data.user.user_metadata?.full_name || full_name || "Apple User",
  });

  const mapLoginDto = mapToLoginDTO(
    { ...data, user: { ...data.user } },
    profile
  ) as Login;
